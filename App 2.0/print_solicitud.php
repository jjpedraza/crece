                        ...
                        if($Cliente = $r2 -> fetch_array()) {                
                           ...
                           //  $htmlPDF.='<b>Interes Moratorio:</b> '.$Sol['tasa_moratorio'].'% mensual<br>';
                            if ($Sol['curp_aval']==''){
                                ...
                                $htmlPDF.='<b>'.Cliente_Nombre($Sol['curp_aval']).'</b><br>';
                                $htmlPDF.='<b>Curp</b>:'.$Sol['curp_aval'].'<br>';
                                $htmlPDF.='<b>Telefono</b>:'.Cliente_Telefono($Sol['curp_aval']).'<br>';
                                $htmlPDF.='<b>Domicilio</b>:'.Cliente_Domicilio($Sol['curp_aval']).'<br>';
                            ...
                            if ($Sol['curp_aval']<>''){
                            $htmlPDF.='Historial Crediticio del Aval:<br><span style="font-size:6pt;">';
                            $rH = $db1->query("select * from cartera_resumen where curp='".$Sol['curp_aval']."'");    
                            if ($db1->query("select * from cartera_resumen where curp='".$Sol['curp_aval']."'") == TRUE){
                                while($H= $rH -> fetch_array()) {  
                                    if ($H['nosol'] == $NoSol){
                                    $htmlPDF.='<b style="text-decoration:underline;">-'.$H['fechasol'].'-<b>'.$H['nosol'].'</b>('.$H['valoracion'].') '.Pesos($H['cantidad']).' Debe: '.$H['DebePagos'].' pagos, '.Pesos($H['Debe']).'</b><br>';
                                    } else {
