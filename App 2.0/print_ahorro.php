...
if (isset($_GET['id'])){
    ...
    if ($NoSol <> ''){
        ...
        a.ahorro,
        a.ahorro_retiro,
        (select sum(ahorro)- sum(ahorro_retiro) from corte where nosol=a.nosol and fecha <= a.fecha) as saldo
        ...
        if ($db1->query($sql) == TRUE){   
            ...
            $htmlPDF.='<td style="background-color: #878787; color:white;">Empleado'.'</td>'; //1                            
            ...
            $htmlPDF.='<td style="background-color: #878787; color:white;">Saldo'.'</td>'; //1                            
            $htmlPDF.='</tr>';
            $c = 0;
