...
var textarea = null;
var config = {border: "1px solid #CCC", bgcolor: "white"};
...
function makeWysiwyg (element) {
  ...
  html = '\
    <div id="barra" style="border: '+style['border']+'; width: 100%;">\
    <span id="'+edit+'" style="background-color:white;">IFRAME</span>\
    ...
    <table id="'+element.id+'_emo" cellspacing="0" style="z-index:3000;   position: relative; left: 30%; bottom: 150px;  visibility: hidden;" onmouseover="cancelTimer(\''+element.id+'_emo\')"; onmouseout="addTimer(\''+element.id+'_emo\');">\
    <tr><td style="border: '+style['border']+'; background: #F0F0EE;  width:100%;">\
  ...
  if (isIE) {
    ...
    doc.designMode = "on";
    doc.open();
    doc.write('<link rel="stylesheet" href="../unica/css_textarea.css" /> ');
...
function addImage(src, editor, command, coment) {
  return '<img src="txtplus/actions/'+src+'" onmouseover="seleccionar(this, \'#B6BDD2\');" onmouseout="desseleccionar(this);" onmousedown="seleccionar(this, \'#8592B5\');" onclick="executeCommand(\''+editor+'\', \''+command+'\', event); desseleccionar(this);" style="border: 1px solid #F0F0EE; width; 2px; height: 20px; cursor: pointer;" alt="'+coment+'" title="'+coment+'">';
}

function addEmoticon (src, editor, coment) {
  return '<img src="txtplus/emoticons/'+src+'" onmouseover="this.style.backgroundColor = \'#B6BDD2\';" onmouseout="this.style.backgroundColor = \'#F0F0EE\';" onclick="executeCommand(\''+editor+'\', \'insertimage\', event, this.src);" style="width; 18px; height: 18px; cursor: pointer;" alt="'+coment+'" title="'+coment+'">';
}

function addSeparator() {
  return '<img src="txtplus/spacer.gif" style="width; 2px; height: 20px;" alt="" title="">';
}
